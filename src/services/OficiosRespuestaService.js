...
const uploadsRoot = path.resolve('uploads', 'oficios-respuesta');
const ANNUAL_MAX_FILES = 10000;
const ANNUAL_FOLDER_PREFIX = 'Oficios compartidos';
...
const OficiosRespuestaService = {
  async asegurarCarpetaAnual(year, userId, connection = null) {
    ...
    const [[existing]] = await db.query(
      `SELECT id
       FROM oficios_respuesta_folders
    ...
    if (existing?.id) {
      await db.query(
        `UPDATE oficios_respuesta_folders
    ...
    const [insert] = await db.query(
      `INSERT INTO oficios_respuesta_folders (parent_id, nombre, folder_type, year_value, month_value, creado_por_id)
  ...
  async obtenerFolder(folderId) {
    const [[row]] = await pool.query(
      `SELECT id, parent_id, nombre, folder_type, year_value, month_value
       FROM oficios_respuesta_folders WHERE id = ? LIMIT 1`,
      [folderId]
    );
    return row || null;
  },
  ...
  async subirArchivo(folderId, file, metadata, userId) {
    ...
    const [[{ total }]] = await pool.query(
      'SELECT COUNT(*) AS total FROM oficios_respuesta_files WHERE folder_id = ?',
      [folderId]
    );
    ...
    const relativePath = path.join('oficios-respuesta', String(folderId), storedName).replace(/\\/g, '/');

    const [result] = await pool.query(
      `INSERT INTO oficios_respuesta_files
  ...
  async obtenerFileById(fileId) {
    const [[row]] = await pool.query(
      `SELECT id, folder_id, original_name, stored_name, relative_path, mime_type, size_bytes, folio, created_at
       FROM oficios_respuesta_files WHERE id = ? LIMIT 1`,
      [fileId]
    );
  ...
  async eliminarArchivo(fileId) {
    ...
    await pool.query('DELETE FROM oficios_respuesta_files WHERE id = ?', [fileId]);
  ...
  async obtenerArchivosPorIds(ids = []) {
...
export default OficiosRespuestaService;
